...
Then('I find total four selected items in my wishlist', async function (this: ICustomWorld) {
  const page = this.page!;
...
When('I search for lowest price product', async function (this: ICustomWorld) {
  ...
  for (let i = 1; i <= wishtableSize; i++) {
    ...
    if (priceRange.includes(' – ')) {
      ...
      const mxV = await page
        .locator('//tr[' + i + "]//td[@class='product-price']//span[2]//bdi")
        .innerText();
      maxValue.push(Number(mxV.replace(/[^0-9.-]+/g, '')));
    } else {
      const mnV = await page
